document.addEventListener("DOMContentLoaded", () => {
  ...
  const applySavedEdits = () => {
    editableElements.forEach((element) => {
      const saved = localStorage.getItem(`editable-${element.dataset.editable}`);
      if (saved) {
  ...
  const enableAdminButton = document.getElementById("enable-admin");
  ...
  const setEditableState = (enabled) => {
    ...
    editableElements.forEach((element) => {
      ...
      element.classList.toggle("editable-active", enabled);
    });
    ...
    updateStatus(enabled ? "Admin mode active" : "Admin mode off");
  };

  const requestAccess = () => {
    const pin = prompt("Enter admin PIN to edit the page (default: zoeedit)");
  ...
  if (enableAdminButton) {
    enableAdminButton.addEventListener("click", () => {
      if (!adminEnabled) {
        requestAccess();
      }
    });
  ...
  if (saveButton) {
    saveButton.addEventListener("click", () => {
      editableElements.forEach((element) => {
        localStorage.setItem(`editable-${element.dataset.editable}`, element.innerHTML);
      });

      updateStatus("Edits saved locally");
    });
  }

  if (resetButton) {
    resetButton.addEventListener("click", () => {
      const confirmed = confirm("Reset all edited text back to the default content?");

      if (!confirmed) return;

      editableElements.forEach((element) => {
        localStorage.removeItem(`editable-${element.dataset.editable}`);
        if (element.dataset.originalContent) {
          element.innerHTML = element.dataset.originalContent;
        }
      });

      updateStatus("Edits reset to defaults");
    });
  }

  if (exitButton) {
    exitButton.addEventListener("click", () => setEditableState(false));
  }
